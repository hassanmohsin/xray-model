...
class PredictorDataset(Dataset):
    def __init__(self, agent, transform, subset="validation", sample_count=None):
        ...
        self.img_dirs = dict(
            zip(
                ...
                [
                    os.path.join(agent.params["dataset_dir"], "validation-set")
    ...
    def __getitem__(self, idx):
        ...
        img = Im.open(
            os.path.join(self.img_dirs[row["dataset_name"]], f"{row['image_id']}.png")
        ).convert("RGB")
        img = self.transform(img)

        return (
            "-".join([row["dataset_name"], row["image_id"]]),
            img,
            torch.tensor(row["success"], dtype=torch.float),
        )
