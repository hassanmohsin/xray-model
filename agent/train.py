...
def train(agent, evaluate_only=True):
    ...
    torch.save({'mean': mean, 'std': std}, os.path.join(agent.model_dir, "norm_data.pth"))
    ...
    model, optimizer, start_epoch = load_checkpoint(
        model,
        optimizer,
        os.path.join(agent.model_dir, "checkpoint-best.pth.tar")
    )

    best_accuracy = 0.0
    for epoch in range(start_epoch, agent.params['epochs'] + 1):  # loop over the dataset multiple times
        ...
        save_checkpoint(
            ...
            is_best,
            filename=os.path.join(agent.model_dir, f'checkpoint-{epoch:03d}-val-{val_accuracy:.3f}.pth.tar')
        )
